% Task 6 % Multiple source ((-0.5 - 0.5)mm,0,0)
clear 
% Input
...
for i = 1:dim(1)
    for j = 1:dim(2)
        for k = 1:dim(3)
            for L = 1:dim(4)
            if p(i,j,k,L) > 0
                p(i,j,k,L) = 1/4/pi/sqrt((10e-6*(i-201))^2+...
                                        (10e-6*(j-1))^2);
            end
            end
        end
...
% Processing s becoming multiple
smult = zeros(401,401,1,401);
for f = -50:50
    smult = smult + circshift(s,[f,0,0,0]);
end
sgrid = smult +gridpt;
